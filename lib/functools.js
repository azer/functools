var functools = (function(undefined){
  ...
  filter.async = function(fn, iterable, callback){
    var accumulation = [];
    (function(i,error,ptest){

        ptest && accumulation.push(iterable[i-1]);

        if(error || i>=iterable.length){
          return callback(error, accumulation);
        }

        fn(iterable[i], i, iterable, partial(arguments.callee, [i+1]));
